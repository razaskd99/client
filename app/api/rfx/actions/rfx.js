...
if (serverRuntimeConfig) {
  ...
  userData = serverRuntimeConfig.PRIVATE_ENCRIPTED_USER_DATA
      ...
      try {
          if (rfxData.attached_documents) {
              for (const item of rfxData.attached_documents) {
                  await createDocUploadAction(result.rfx_id, rfxData.initiator_id, item);
              }
          }
      ...
      try {
          if (rfxData.key_contacts) {
              for (const item of rfxData.key_contacts) {
                  await createContactsAction(result.rfx_id, item.user_id, item.contact_key);
              }
          }
      } catch (contactError) {
