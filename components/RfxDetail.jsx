...
import { createRfxClarificationAction, getAllRfxClarificationRecordsBy_RfxID_Action, createRfxClarificationCommentAction, getAllRfxClarificationPostRecordsBy_ClarifId_Action, getRfxClarificationRecordByIDAction } from '@/app/api/manager/actions/clarifications';
import { createReviewEntryAction, createReviewContactsAction, getAllReviewContactsBy_BidReviewID_Action, getAllReviewsRecordsBy_Rfx_Key_Action, getAllBidReviewPostRecordsById } from '@/app/api/manager/actions/reviews';
...
const RfxDetail = ({ data, rfxRecord, stagesList, apiBackendURL, login_user_id, tenantID, keyContactsRec, assigntoRec, initiatorRec, allUsersRec, clarificationRec, submissionRec, bidClarifRec, bidOrderRec }) => {
    ...
    const [orderAcknowledgementDeadline, setOrderAcknowledgementDeadline] = useState('')
    const [orderAcknowledgementComment, setOrderAcknowledgementComment] = useState('')
    ...
    const [contacts, setContacts] = [
        allUsersRec && allUsersRec.map(record => ({
            id: record.user_id,
            name: `${record.first_name} ${record.last_name}`,
            role: record.team_role,
            designation: record.designation_title,
            image: record.user_profile_photo ? record.user_profile_photo : '/avatar.jpg'
        }))
    ];
    ...
    const handleRowClick = async (rowId) => {
        ...
        if (active == 'Bid Submission' || active == 'Bid Documents' || active == 'Bid Acknowledgement') {
        ...
        if (active == 'Order') {
            ...
            const r1 = await GetRfxDocumentsBy_RfxID_Key_Action(rfxRecord.rfx_id, 'bid-order-' + bid_order_id)
    ...
    const revisionRows = [
        { id: 1, checkbox: 'dddd.png', description: 'New Opportunity 1', crmId: 'newCrmId_1', customer: 'New Customer 1', issueDate: 'New Date 1', salePerson: 'New Salesperson 1', status: 'New Status 1', },
        { id: 2, checkbox: 'dddd.png', description: 'New Opportunity 2', crmId: 'newCrmId_2', customer: 'New Customer 2', issueDate: 'New Date 2', salePerson: 'New Salesperson 2', status: 'New Status 2', },
        { id: 3, checkbox: 'dddd.png', description: 'New Opportunity 3', crmId: 'newCrmId_3', customer: 'New Customer 3', issueDate: 'New Date 3', salePerson: 'New Salesperson 3', status: 'New Status 3', },
    ];
        ...
        if (rowIdSelected && rowIdSelected > 0) {
            ...
            if (r1.statusCode == 200 && selectedFilesMain.length > 0) {
                ...
                for (var i = 0; i < selectedFilesMain.length; i++) {
                    ...
                    let r2 = await createDocUploadAction(rfxRecord.rfx_id, login_user_id, file, 'bid-clarifications-post-' + clarif_post_id)
                }
            }
            // get bid clarification post
            const r3 = await getAllBidClarificationPostRecordsBy_ClarifId_Action(rowIdSelected)
    ...
    const handleAddOrderRow = async () => {
        ...
        if (r1.statusCode == 200 && selectedOrderContact.length) {
            ...
            for (var i = 0; i < selectedOrderContact.length; i++) {
                let r2 = await createContactsAction(rfxRecord.rfx_id, selectedOrderContact[i].id, 'bid-order-' + bid_order_id)
            }
        }
        // upload documents
        if (r1.statusCode == 200 && selectedFilesMain.length > 0) {
            uploadFiles(selectedFilesMain, apiBackendURL, tenantID, rfxRecord.rfx_id, 'bid-order')
            for (var i = 0; i < selectedFilesMain.length; i++) {
                const file = {
                    name: selectedFilesMain[i].name,
                    type: selectedFilesMain[i].type,
                ...
                let bid_order_id = r1.returnData.bid_order_id
                let r3 = await createDocUploadAction(rfxRecord.rfx_id, login_user_id, file, 'bid-order-' + bid_order_id)
            }
        }
        // get all order
        ...
        <>
            ...
            <div className="Content bg-white">
                        ...
                        <div className="flex-[1] flex flex-col">
                            ...
                            <div className="border mb-3 rounded-md">
                                ...
                                <div className="bg-[#F4F5F6] py-6 flex items-center flex-col gap-3">
                                    ...
                                    <UploadDialog open={open} handleClose={handleClose} onYesClick={rfxYesClick} handleTextChange={handleDailogTextChange} textValue={DailogtextValue} rfxID={rfxRecord.rfx_id} tenantID={tenantID} apiBackendURL={apiBackendURL} dailogTitle={"Title"} />
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div className="flex flex-col-reverse">
                                    <div className="border mb-3 rounded-md ">
                                        ...
                                        <div className="bg-[#F4F5F6] p-6 flex flex-col gap-5">
                                            ...
                                            {!showReply && <button className='bg-[#26BADA] p-3 max-w-[200px] text-white rounded-sm' onClick={setShowReply}>REPLY</button>}                                    </div>
                                    </div>
                                            ...
                                            submissionPostList.map((item, index) => (
                                                item.posted_by == login_user_id
                                                    ...
                                                    <div className="flex items-center gap-1 my-3">
                                                        <Image src={item.user_profile_photo ? item.user_profile_photo : '/avatar.jpg'} width={31} height={31} className='mr-1 rounded-full object-cover w-9' />
                                                        <p className='bg-[#F2F4F6] px-4 py-2 rounded-full max-w-[80%] pr-[20%]'>{item.comment} </p>
                                                        <span className='uppercase text-[#98A9BC] text-xs'>{formatDatetime(item.posted_on)}</span>
                                                    </div>
                                                    :

                                                    <div className="flex items-center flex-row-reverse gap-1 my-3">
                                                        <Image src={item.user_profile_photo ? item.user_profile_photo : '/avatar.jpg'} width={31} height={31} className='mr-1 rounded-full object-cover w-9' />
                                                        <p className='bg-[#98A9BC] text-white px-4 py-2 rounded-full max-w-[80%] pr-[20%]'>{item.comment}</p>
                                                        <span className='uppercase text-[#98A9BC] text-xs'>{formatDatetime(item.posted_on)}</span>
                                                    </div>
                                            ))
                                        }
                ...
                {active === 'Bid Clarifications' && <div>
                    {bidTable && <div className='p-5 bg-white shadow-sm h-full min-h-screen'>
                        ...
                        <div className="flex justify-start items-center gap-4">
                            <button className={`upprecase my-5 rounded-md p-3  bg-[#00AAEC] text-white cursor-pointer min-w-[200px]`} onClick={() => { setOpenRevisionDailog(true); }}  >REQUEST BID REVISION</button>
                            <button className={`upprecase my-5 rounded-md p-3  bg-[#00AAEC] text-white cursor-pointer min-w-[200px]`}  >ACCEPT BID</button>
                            <RevisionDialog open={openRevisionDailog} handleClose={handleRevClose} rows={bidClarificationRows} dailogTitle={"Revision Request"} login_user_id={login_user_id} setBidClarificationRows={setBidClarificationRows} setBidRevisionRows={setBidRevisionRows}/>
                        ...
                        <div className="flex-[2]">
                            ...
                            <div className="border mb-3 rounded-md">
                                <div className="bg-[#31282805] py-2 px-[14px] text-[#778CA2] flex justify-between items-center" >
                                    <span>Assign to</span>
                                    <span className='text-[#00AAEC] text-sm uppercase flex gap-1 items-center cursor-pointer' onClick={handleClickOpenContact}>Reassign <IoMdAddCircleOutline /></span>
                                </div>
                                <BidDialog openBid={openContactAssign} handleBidClose={handlCloseContact} onYesButtonClick={onYesButtonClick} keyContactsRec={allUsersRec} setPersonAssignTo={setPersonAssignTo} />
                                <div key={333} className="bg-[#F4F5F6] py-3 px-4 flex items-center justify-between">
                                    {personAssignTo.name && <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full">
                                        <Image src={personAssignTo.image ? personAssignTo.image : '/avatar.jpg'} alt={personAssignTo.name} width={38} height={38} className="rounded-[100%] object-cover w-[38px] h-[38px]" />
                                        <div>
                                            <span className="text-sm leading-4">{personAssignTo.name}</span>
                                            <span className="text-sm leading-4 text-[#778CA2] block">{personAssignTo.designation}</span>
                                        </div>
                                    </div>}
                                </div>
                        ...
                        <div className="flex flex-[3] flex-col p-5">
                            ...
                            <div className="flex flex-col-reverse">
                                <div className="border mb-3 rounded-md ">
                                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between " >
                                        <p>#1</p>
                                        <p>Posted by <span className='text-[#00AAEC]'>{bidClarifSubmittedBy.first_name} {bidClarifSubmittedBy.last_name}</span> on {formatDatetime(bidClarifSelectedRow.completed_on)}</p>
                                    </div>
                                    <div className="bg-[#F4F5F6] p-6 flex flex-col gap-5">
                                        <p>{bidClarifSelectedRow.description}</p>
                                        <p className='text-[#778CA2] mt-3'>Attached Documents</p>

                                        {bidClarifSelectedDocuments.map((doc, index) => (<div className="bg-[#E8ECEF] shadow-sm flex items-center w-full p-2 justify-between mb-3 border-b border-[#babec2]" >
                                            {
                                    ...
                                    <div className="border mb-3 rounded-md " key={index}>
                                        ...
                                        <div className="bg-white p-6 flex flex-col gap-5">
                                            ...
                                            {bidClarifPostDocsRows
                                                .filter(item => item.docvalt_key === 'bid-clarifications-post-' + post.bid_clarification_post_id)
                                                .map((file, fileIndex) => (
                                                    <div key={fileIndex} className="shadow-sm flex items-center w-full p-2 justify-between mb-3 border-b border-[#babec2]">
                                                        {
                                                            ['jpg', 'png', 'jpeg'].includes(file.file_type)
                                                                ?
                                                                <FaRegFileImage className='text-[#00AAEC]' />
                                                                :
                                                                <FaRegFilePdf className='text-red-600' />
                                                        }
                                                        <span className=''>{file.docvalt_filename}</span>
                                                        <span className='text-[#98A9BC]'>{file.file_size}</span>
                                                        <span className='text-[#98A9BC]'>{formatDateString(file.created_at)}</span>
                                                        <div className="flex items-center gap-2 text-[#98A9BC]">
                                                            <MdOutlineModeEdit className='cursor-pointer' />
                                                            <RiDeleteBin6Line className='cursor-pointer' />
                ...
                {(active === 'Bid Revision') && <div>
                    {showRevisionTable && <div className='p-5 bg-white shadow-sm h-full min-h-screen'>
                        <div className="flex justify-end uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                            {/* <div className="flex items-center gap-1" onClick={addClarificationRow}>
                                <span>New Documents</span>
                                <IoMdAddCircleOutline />
                        ...
                        {showRevisionTable &&
                            <SearchTableNew rows={bidRevisionRows} handleRowClick={handleRowClick} NoRowsOverlay={NoRowsOverlayDocuments} />
                        }
                    </div>
                    }
                    {revisionDetail &&
                        <div className="flex gap-6">
                        <div className="flex flex-[3] flex-col p-5">
                            <div className=" flex items-center gap-1 text-[#00AAEC] cursor-pointer" onClick={()=>{setShowRevisionTable(true); setRevisionDetail(false);}}>
                                <FaChevronLeft />
                                <span> Back to List</span>
                            </div>
                            <p className='text-xl p-2'>{bidClarifSelectedRow.title}</p>
                ...
                {active === 'Order' && <div>
                    {orderTable &&
                        <>
                            <div className="flex justify-between px-3">
                                <div className="flex items-center gap-1 text-[#26BADA] cursor-pointer" onClick={() => { setAddOrderForm(true); setOrderTable(false); setOrderAssignedTo({}); setSelectedOrderContacts([]) }}>
                                    <span>New Order</span>
                                    <IoMdAddCircleOutline />
                                </div>
                            ...
                            <SearchTableNew handleRowClick={handleRowClick} rows={bidOrderRows} NoRowsOverlay={NoRowsOverlayOrder} />
                        </>
                    }
                    {addOrderForm && <div className="flex gap-3 p-5">
                        <div className="flex-[2] p-5 flex flex-col gap-8">
                            ...
                            ></textarea>
                            <DragDrop className="w-full" setSelectedFilesMain={setSelectedFilesMain} setAttachedDocuments={setAttachedDocuments} />
                            <div className=" flex gap-3">
                                <button className='bg-white border border-[#26BADA] text-[#26BADA] uppercase text-sm px-8 py-3 min-w-[200px] rounded-sm '>Cancel</button>
                                <button className='bg-[#26BADA] border border-[#26BADA] text-white uppercase text-sm px-8 py-3 min-w-[200px] rounded-sm ' onClick={() => { handleAddOrderRow();  }}>SUBMIT</button>
                                <UploadDialog open={open} handleClose={handleClose} dailogTitle={"Set title"} />
                            </div>
                        </div>
                        <div className="flex-[1]">
                            <div className="border mt-[18px] mb-3 rounded-md">
                                        ...
                                        <DemoContainer components={["DatePicker"]}>
                                            <div id={"clarification_issued_date"}>
                                                <DatePicker
                                                    ...
                                                    onChange={(date) => setOrderIssuedDate(new Date(date).toISOString().slice(0, 10))}
                                                />
                                            </div>
                                        </DemoContainer>
                                    </LocalizationProvider>
                                        ...
                                        <DemoContainer components={["DatePicker"]}>
                                            <div id={"clarification_issued_date"}>
                                                <DatePicker
                                                    ...
                                                    onChange={(date) => setOrderDeliveryDate(new Date(date).toISOString().slice(0, 10))}
                                                />
                                            </div>
                                        </DemoContainer>
                                    </LocalizationProvider>
                                    <LocalizationProvider key={"clarification_issued_date1"} dateAdapter={AdapterDayjs} className="w-full bg-white block ">
                                        <DemoContainer components={["DatePicker"]}>
                                            <div id={"clarification_issued_date"}>
                                                <DatePicker
                                                    ...
                                                    onChange={(date) => setOrderAcknowledgementDeadline(new Date(date).toISOString().slice(0, 10))}
                                                />
                                            </div>
                                        </DemoContainer>
                                    </LocalizationProvider>
                            ...
                            <div className="border mb-3 rounded-md">
                                <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between items-center" >
                                    <span>Assign to</span> 
                                    <span className='text-[#00AAEC] text-sm uppercase flex gap-1 items-center cursor-pointer' onClick={()=>{setOpenOrderContactAssign(true)}}>Reassign <IoMdAddCircleOutline /></span>
                                </div>
                                <div className="bg-[#F8FAFB] flex flex-col gap-3 py-4 px-6 rounded-b-md items-center w-full">
                                    <BidDialog openBid={openOrderContactAssign} handleBidClose={handlCloseOrderContact} onYesButtonClick={onYesButtonClick} keyContactsRec={allUsersRec} setPersonAssignTo={setOrderAssignedTo} />
                                    {orderAssignedTo 
                                    ?
                                    <div key={333} className="bg-[#F4F5F6] py-3 px-4 flex items-center justify-between w-full">
                                        <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full">
                                            <Image src={orderAssignedTo.image ? orderAssignedTo.image : '/avatar.jpg'} alt={orderAssignedTo.name} width={38} height={38} className="rounded-[100%] object-cover w-[38px] h-[38px]" />
                                            <div>
                            ...
                            <div className="border mb-3 rounded-md">
                                <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between items-center " >
                                    ...
                                    <span className='text-[#00AAEC] text-sm uppercase cursor-pointer' onClick={() => setOrderContactDailog(true)} >Add Contacts</span>
                                    <ContactDialog isOpen={orderContactDailog} handleClose={() => setOrderContactDailog(false)} handleContactSelect={handleOrderContactSelect} contacts={contacts} />
                                </div>
                                <div className="bg-[#F4F5F6] p-1">
                                    {selectedOrderContact.length === 0 ? (
                                        ...
                                        selectedOrderContact.map((contact, index) => (
                                            <div className="bg-[#F4F5F6] px-4 py-2 flex  items-center justify-between" key={index}>
                                                <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full">
                                                    <Image src={contact.image} width={38} height={38} className="rounded-[100%] object-cover w-[38px] h-[38px]" alt='user' />
                                                    <div className="">
                                                        <span className="text-sm leading-4">{contact.name}</span>
                                                        <span className="text-sm leading-4 text-[#778CA2] block">Designer</span>
                                                    </div>
                                                </div>
                                                {contact.role && <div className="bg-[#26BADA] h-max rounded-md text-xs text-white p-1 min-w-[80px] text-center ">{contact.role}</div>}
                                            </div>
                                        ))
                                    )}
                                </div>
                    ...
                    {orderDetail && <div className="flex gap-6">
                        <div className="flex flex-[3] flex-col p-5">
                        ...
                        <div className="flex-[2]">
                            <div className="border mb-3 rounded-md">
                                <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] " >Critical Dates</div>
                                <div className="bg-[#c3eff92f] px-4 py-1 flex flex-col  ">
                                    <div className="flex items-center justify-between w-full p-4">
                                        <div>
                                            <span className='text-[#778CA2]'>Issue Date</span> <br />
                                        ...
                                        <div>
                                            <span className='text-[#778CA2]'>Delivery Date</span> <br />
                                            <span>{formatDateString(bidOrderSelectedRow?.delivery_date)}</span>
                                        </div>
                                    </div>
                                    <hr />
                                    <div className="flex items-center justify-between w-full p-4">
                                        ...
                                        <div>
                                            <span className='text-[#778CA2]'>Order Acknowldegement Deadline</span> <br />
                            ...
                            <div className="border mb-3 rounded-md">
                                <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] mt-4 " >Order Contacts</div>
                                {bidOrderSelectedContacts.map((item, index) => (
                                        <div className="bg-[#F4F5F6] px-4 py-1 flex  items-center justify-between" key={index}>
                                            <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full">
                                                ...
                                                <div className="">
                                                    <span className="text-sm leading-4">{item.first_name} {item.last_name}</span>
                                                    <span className="text-sm leading-4 text-[#778CA2] block">{item.designation_title}</span>
                                                </div>
                                            </div>
                                            {item.team_role && <div className="bg-[#26BADA] h-max rounded-md text-xs text-white p-1 min-w-[80px] text-center ">{item.team_role}</div>}
                                        </div>
                                    ))}
                            </div>
                            <div className="border mb-3 rounded-md">
