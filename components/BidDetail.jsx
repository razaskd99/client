...
import { getBidClarificationRecordsByIdAction, getAllBidClarificationPostRecordsBy_ClarifId_Action, createBidClarificationCommentAction } from '@/app/api/manager/actions/bidclarifications';
...
const BidDetail = ({
    ...
    const [templateHTMLPrelim, settemplateHTMLPrelim] = useState('')
    const [templateHTMLDetailed, settemplateHTMLDetailed] = useState('')
    const [templateHTMLFinal, settemplateHTMLFinal] = useState('')
    ...
    const handlePrelimSelectChange = async (event) => {
        ...
        settemplateHTMLPrelim(pageHTML)
    ...
    const handleDetailSelectChange = async (event) => {
        ...
        settemplateHTMLDetailed(pageHTML)
        setTemplateCSS(pageCSS)
    ...
    const handleFinalSelectChange = async (event) => {
        ...
        settemplateHTMLFinal(pageHTML)
        setTemplateCSS(pageCSS)
    };
    const handleReplySubmitClarif = async () => {
        const data = {
    ...
    const handleClarifContactSelect = (contact) => {
        console.log("CON:", contact)
        const matchingUser = selectedContact.find(user => user.user_id === contact.user_id);
    ...
    const handleRowClick = async (rowId) => {
        ...
        if (active == 'Pre-lim Review') {
            ...
            settemplateHTMLPrelim(review.template_data)
        }
        if (active == 'Detailed Review') {
            ...
            let r2 = await getReviewRecordByIdAction(rowId)
            let review = r2.returnData
            settemplateHTMLDetailed(review.template_data)
        }
        if (active == 'Final Review') {
            ...
            let r2 = await getReviewRecordByIdAction(rowId)
            let review = r2.returnData
            settemplateHTMLFinal(review.template_data)
        }
        ...
        if (active == 'Submission') {
            ...
            const r3 = await getUserById(targetSubmission.assign_to_id)
            let user = r3.data
            setBidSubAssignto(user)
    ...
    const [bidClarifRows, setBidClarifRows] = useState(
    ...
    const addRfxClarRow = async () => {
        ...
        //create contacts
        if (r1.statusCode == 200 && selectedContact.length) {
            let clarif_id = r1.returnData.rfx_clarification_id
            for (var i = 0; i < selectedContact.length; i++) {
                let r3 = await createContactsAction(rfxRecord.rfx_id, selectedContact[i].user_id, 'rfx-clarifications-' + clarif_id)
            }
        }
    ...
    const addSubmissionRow = async () => {
        ...
        const data = {
            ...
            "assign_to_id": bidSubAssignto.id ? bidSubAssignto.id : 0,
            "submitted_by": login_user_id,
    ...
    const handleBidClarifReplySubmit = async () => {

        if (!replyMessageBidClar) {
            alert('Please write comment to reply.');
            return;
        }
        setBidClarifPostRows([])
        const data = {
    ...
    const NoRowsOverlayOrder = () => (
        <Stack height="100%" alignItems="center" justifyContent="center">
            ...
            <p className='text-[#778CA2] text-lg font-light mb-3'>You can see order requests here</p>
            {/*<div className="flex items-center gap-6">
                <button className='text-white bg-[#26BADA] p-3 uppercase  min-w-[200px]' onClick={() => { { setShowNewOrderForm(true); setShowOrderTable(false) } }}>Add Order</button>
    ...
    const NoRowsFinalReview = () => (
        ...
        <Stack height="100%" alignItems="center" justifyContent="center">
            ...
            <p className='text-[#778CA2] text-lg font-light mb-3'>You have not created any review/approval requests</p>
            <div className="flex items-center gap-6">
                <button className='text-white bg-[#26BADA] p-3 uppercase  min-w-[200px]' onClick={() => { handleshowFinalTemplateSelect(); setShowFinalRewList(false); setInFinalTemp(false) }}>Request Review</button>
                <button className='text-white bg-[#26BADA] p-3 uppercase  min-w-[200px]' onClick={handleSkipClick}>Skip Review</button>
            </div>
            <SkipDialog
                open={skipDialogOpen}
    ...
    const NoRowsOverlayBidClarification = () => (
        ...
        <Stack height="100%" alignItems="center" justifyContent="center">
            <Image src="/no-row.png" width={480} height={260} />
            <p className='text-[#252631] text-xl mb-3'>No requests yet</p>
            <p className='text-[#778CA2] text-lg font-light mb-3'>You have no Bid Clarification request</p>
            {/*<div className="flex items-center gap-6">
    ...
    const NoRowsRevision = () => (

        <Stack height="100%" alignItems="center" justifyContent="center">
            <Image src="/no-row.png" width={480} height={260} />
            <p className='text-[#252631] text-xl mb-3'>No revision submitted</p>
            <p className='text-[#778CA2] text-lg font-light mb-3'>You have no any revision requests</p>
        ...
        <div>
            ...
            <div className="Content bg-white">
                    ...
                    {(showTemplateSelect && !loadTemplate) && (
                        <div className='flex flex-col gap-4 mt-5'>
                            ...
                            <TextField
                                ...
                                onChange={handlePrelimSelectChange}
                    ...
                    {!clarificationDetail &&
                        <>
                            <div className="flex justify-between uppercase text-[#00AAEC] text-sm mb-4 cursor-pointer ">
                                ...
                                <div className="flex items-center gap-1" onClick={() => { setClarificationDetail(true); setShowSubmitedRfxCalrification(false); setSelectedContacts([]); }}>
                            ...
                            <div className="flex-[1]">
                                ...
                                <div className="border mb-3 rounded-md">
                                    <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between items-center" >
                                        <span>Assign to</span>
                                        <span className='text-[#00AAEC] text-sm uppercase flex gap-1 items-cente cursor-pointer' onClick={handleClickOpenContact}>Reassign <IoMdAddCircleOutline /></span>
                                    </div>
                                ...
                                <div>
                                    <div className="border mb-3 rounded-md">
                                        <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-between">
                                            <p>Key Contacts</p>
                                            <p className='uppercase text-[#26BADA] text-xs flex items-center gap-2 cursor-pointer' onClick={handleAddTeam}>
                                                Add Contacts <IoMdAddCircleOutline />
                                            </p>
                                        </div>
                                        {selectedContact.map((selectedContact, index) => (
                                            <div className="bg-[#F4F5F6] px-4 py-1 flex items-center justify-between" key={index}>
                                    ...
                                    <ContactDialog isOpen={isContactDialogOpen} handleClose={() => setContactDialogOpen(false)} handleContactSelect={handleClarifContactSelect} users={allUsersRec} />
                                </div>
                            </div>
                        </div>
                    }
                                ...
                                {showReply && (
                                    <div className="flex flex-col gap-[18px]">
                                        ...
                                        <button
                                            ...
                                            onClick={handleReplySubmitClarif}
                                        >
                                            SUBMIT REPLY
                                        </button>
                                    </div>
                ...
                {active === 'Final Review' && <div className='p-5 bg-white shadow-sm h-full min-h-screen'>
                    ...
                    {(!showFinalRewList && finalTemplate) && (
                        <div className="flex">
                    ...
                    {(showSubmissionTempSelect && !submissionTemplate) && (
                        <div className='flex gap-4 mt-5'>
                            ...
                            <div className="flex-[1]">
                                ...
                                <div className="border mb-3 rounded-md">
                                    <div className="border mb-3 rounded-md">
                                        <div className="bg-[#00000005] py-2 px-[14px] text-[#778CA2] flex justify-start" >
                                            <p>Submit to</p>
                                        </div>
                                        <div className="bg-[#F4F5F6] py-3 px-4 flex   items-center justify-between">
                                            <AssignPopupInput label={"Assign to"} setBidSubAssignto={setBidSubAssignto} allUsersRec={allUsersRec} rfx_bid_assignto={rfxRecord.rfx_bid_assignto} rfxID={rfxRecord.rfx_id} className="w-full" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {submissionDetail && <div className="flex gap-6">
                        ...
                        <div className="flex-[2]">
                            ...
                            <div className="border mb-3 rounded-md">
                                ...
                                {bidSubAssignto && <div className="bg-[#F4F5F6] py-3 px-4 flex   items-center justify-between">
                                    <div className="flex flex-[3] bg-white border rounded-[30px] p-1 gap-2 items-center max-w-[60%] w-full">
                                        <Image src='/man.jpeg' width={38} height={38} className="rounded-[100%] object-cover w-[38px] h-[38px]" />
                                        <div className="">
                ...
                {active === 'Bid Clarifications' && <div className='p-5'>
                    {!showBidClarificationDetial &&
                        <>
                            <div className="flex justify-end">
                                <button className={`upprecase uppercase max-w-[200px] mb-3 rounded-md p-2 flex items-center gap-1  ${submissionrows ? 'bg-[#00AAEC] text-white' : 'bg-[#EFF3F5] text-[#98A9BC]'}`} onClick={handleChangeStatus} disabled={!submissionrows} >Proceed <FaArrowRight /> </button>
                            </div>
                            <SearchTableNew handleRowClick={handleRowClick} rows={bidClarifRows} NoRowsOverlay={NoRowsOverlayBidClarification} />
                        </>
                    }
                ...
                {(active === 'Order Handover' || active === 'Close') && <div>
